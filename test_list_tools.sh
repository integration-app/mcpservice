...
# Now send the list tools request
echo "Sending list tools request..."
RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d "{\"sessionId\":\"${SESSION_ID}\",\"message\":{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\",\"params\":{}}}" http://localhost:3000/messages)
